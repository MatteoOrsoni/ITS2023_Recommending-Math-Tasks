# -*- coding: utf-8 -*-
"""
Created on Thu Dec 29 16:00:40 2022
...
difficulty = pd.read_csv('Difficulty_Gleichungssysteme.csv')
...
threshold = [18.7, 27, 38, 48]
ability_threshold = [-1.24, 0.03, 0.75, 3,22]
max_score = [1.5, 1, 18, 1, 2, 2, 7, 5.4, 4, 10]
...
class Gleichungssysteme(gym.Env):
...
env = Gleichungssysteme(env_config)
env.reset()
env.step(0)
...
Gleichungssysteme_env = Gleichungssysteme(env_config)
...
agent = PPO(policy=RL_config['policy'], env= Gleichungssysteme_env, verbose=1,
...
agent_A2C = A2C(policy=RL_config['policy'], env= Gleichungssysteme_env, verbose=1,
            learning_rate = 0.000001,
            policy_kwargs= dict(net_arch=[64, 32]),
            tensorboard_log= f"tensorboard_logs/{run.id}"
            )
