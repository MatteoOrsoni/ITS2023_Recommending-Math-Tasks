...
difficulty = pd.read_csv('Difficulty_Mengen.csv')
...
threshold = [10.5, 23, 25]
ability_threshold = [-1.05, 0, 2]
max_score = [4,6,4,6,1,2,2,2]
...
agent = PPO(policy=RL_config['policy'], env= Mengen_env, verbose=1,
...
agent_A2C = A2C(policy=RL_config['policy'], env= Mengen_env, verbose=1,
            learning_rate = 0.000001,
            #policy_kwargs= dict(net_arch=[64, 32]),
            tensorboard_log= f"tensorboard_logs/{run.id}"
            )
