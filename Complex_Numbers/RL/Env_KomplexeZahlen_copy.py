...
difficulty = pd.read_csv('Difficulty_KomplexeZahlen.csv')
...
threshold = [10.5, 16, 26]
ability_threshold = [-0.15, 0.63, 2.20]
max_score = [7,2,3,1,5,4,4,6]
...
KomplexeZahlen_env = KomplexeZahlen(env_config)
...
agent = PPO(policy=RL_config['policy'], env= KomplexeZahlen_env, verbose=1,
...
agent_A2C = A2C(policy=RL_config['policy'], env= KomplexeZahlen_env, verbose=1,
            learning_rate = 0.000001,
            policy_kwargs= dict(net_arch=[128, 64]),
            tensorboard_log= f"tensorboard_logs/{run.id}"
            )
