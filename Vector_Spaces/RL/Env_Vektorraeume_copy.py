...
difficulty = pd.read_csv('Difficulty_Vektorraeume.csv')
...
threshold = [14.525, 17.31, 23, 31.5]
ability_threshold = [-0.27, 0.33, 1.12, 2.62]
max_score = [2,2,3,2,3,1,2,2,1,2,3,4.5,9,5]
...
Vektorraeume_env = Vektorraeume(env_config)
...
agent = PPO(policy=RL_config['policy'], env= Vektorraeume_env, verbose=1,
...
run = wandb.init(
    project="Vektorraeume_A2C",
    config= RL_config,
    sync_tensorboard=True,  # automatically upload SB3's tensorboard metrics to W&B
    #save_code=True,
)

agent_A2C = A2C(policy=RL_config['policy'], env= Vektorraeume_env, verbose=1,
            learning_rate = 0.000001,
            policy_kwargs= dict(net_arch=[64, 32]),
            tensorboard_log= f"tensorboard_logs/{run.id}"
            )
