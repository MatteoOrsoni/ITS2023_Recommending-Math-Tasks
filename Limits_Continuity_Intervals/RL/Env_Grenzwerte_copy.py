...
difficulty = pd.read_csv('Difficulty_Grenzwerte.csv')
...
threshold = [10.5, 22.4, 32.2]
ability_threshold = [-0.75, 0.33, 1.17]
max_score = [6,3,9,4,5,3.5,3,6]
...
Grenzwerte_env = Grenzwerte(env_config)
...
agent = PPO(policy=RL_config['policy'], env= Grenzwerte_env, verbose=1,
...
agent_A2C = A2C(policy=RL_config['policy'], env= Grenzwerte_env, verbose=1,
            learning_rate = 0.000001,
            #policy_kwargs= dict(net_arch=[64, 32]),
            tensorboard_log= f"tensorboard_logs/{run.id}"
            )
