...
difficulty = pd.read_csv('Difficulty_Summen.csv')
...
env_config = {
    ...
    "th_level" : np.array(list(range(0,2)))
}
...
agent = PPO(policy=RL_config['policy'], env= Summen_env, verbose=1,
...
agent_A2C = A2C(policy=RL_config['policy'], env= Summen_env, verbose=1,
            learning_rate = 0.000001,
            #policy_kwargs= dict(net_arch=[64, 32]),
            tensorboard_log= f"tensorboard_logs/{run.id}"
            )
