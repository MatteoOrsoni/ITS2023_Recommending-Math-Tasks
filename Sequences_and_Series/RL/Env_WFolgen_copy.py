# -*- coding: utf-8 -*-
"""
Created on Thu Dec 29 11:11:41 2022
...
difficulty = pd.read_csv('Difficulty_Folgen.csv')
...
threshold = [10.5, 13, 15] 
ability_threshold = [0.63, 1.05, 2.02] # Estimated from IRT
max_score = [3,3,3,2,7]
...
agent = PPO(policy=RL_config['policy'], env= Folgen_env, verbose=1,
...
agent_A2C = A2C(policy=RL_config['policy'], env= Folgen_env, verbose=1,
            learning_rate = 0.000001,
            policy_kwargs= dict(net_arch=[64, 32]),
            tensorboard_log= f"tensorboard_logs/{run.id}"
            )
