...
difficulty = pd.read_csv('Difficulty_Matrizenoperationen.csv')
...
threshold = [10.5, 24.3, 33, 40]
ability_threshold = [-2.68, -1, 0, 1]
max_score = [1.5,1,1.5,1,3,3,6,7,5,5,3,5,5]

reward_depleted = -1 # Original -1
reward_done = 100 # Original 100
...
class Matrizenoperationen(gym.Env):
...
env = Matrizenoperationen(env_config)
env.reset()
env.step(0)
...
Matrizenoperationen_env = Matrizenoperationen(env_config)
...
run = wandb.init(
    project="Matrizenoperationen_PPO",
    config= RL_config,
...
agent = PPO(policy=RL_config['policy'], env= Matrizenoperationen_env, verbose=1,
...
run = wandb.init(
    project="Matrizenoperationen_A2C",
    config= RL_config,
    sync_tensorboard=True,  # automatically upload SB3's tensorboard metrics to W&B
    #save_code=True,
)
...
agent_A2C = A2C(policy=RL_config['policy'], env= Matrizenoperationen_env, verbose=1,
            learning_rate = 0.000001,
            #policy_kwargs= dict(net_arch=[64, 32]),
            tensorboard_log= f"tensorboard_logs/{run.id}"
            )
