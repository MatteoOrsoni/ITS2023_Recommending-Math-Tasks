# -*- coding: utf-8 -*-
"""
Created on Tue Dec 20 14:04:16 2022
...
os.chdir('set your own directory')

difficulty = pd.read_csv('Difficulty_W1.csv')
...
threshold = [10.5, 15.5, 25.9, 28.3]
ability_threshold = [-2.80, -1.96, 0.03, 0.57] # Estimated from IRT
max_score = [1, 1, 1, 1, 1, 6, 3, 6, 3, 3, 4]


reward_depleted = -1 # It is possible to try with an higher negative reward e.g. -10
reward_done = 100

env_config = {
    "dataset": score_data,
    "difficulty": difficulty.values[0],
    "th_level" : np.array(list(range(0,4)))
}
...
First_week_env = First_week(env_config)
...
agent = PPO(policy=RL_config['policy'], env= First_week_env, verbose=1,
...
##### One single suggestion for the second threshold

env_config_1 = {
    "dataset": score_data,
    "difficulty": difficulty.values[0],
    "th_level" : np.array(list(range(1,2)))
}

env = First_week(env_config_1)
...
agent_A2C = A2C(policy=RL_config['policy'], env= First_week_env, verbose=1,
            learning_rate = 0.000001,
            policy_kwargs= dict(net_arch=[128, 64]),
            tensorboard_log= f"tensorboard_logs/{run.id}"
            )
